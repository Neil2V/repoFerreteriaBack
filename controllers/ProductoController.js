...
function registrar(req,res){
    ...
    if(req.files){
        ...
        var producto = new Producto();//crear producto
        producto.nombre = data.nombre;
        producto.unidad_venta = data.unidad_venta;
...
function editar(req,res){
    ...
    if(req.files.imagen){
        ...
        Producto.findByIdAndUpdate({_id:id},{nombre: data.nombre, unidad_venta: data.unidad_venta, imagen:imagen_name, precio_compra: data.precio_compra, precio_venta: data.precio_venta,  idcategoria: data.idcategoria}, (err, producto_edit) =>{
            if(err){
                res.status(500).send({message: 'Error en el servidor'});
            }else{
                if(producto_edit){
